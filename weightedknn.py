...
def termfreq(News_Document,News_Labels):
    ...
    rangeval=[]
    rangeval=News_Document[1].split()
        ...
        if(x not in (0,1)):
            ...
            for y in (News_Document[x].split()):
                ...
                if(count==1):
                    term = int(y)
                if(count==2):
                    documents[doc_id-1][term-1]= int(y) #as python reads from zero, we are subtracting docid minus one to read the data
...
def getNeighbors(train_set, testInstance, k): #returns ks most similar neighbours from training set for a test instance
    ...
    for x in range(k):
        ...
        try:
            exec(repr(1/distances[x][2])) #we are using invesere distance weighting here
            weights.append(1/distances[x][2])
        except ZeroDivisionError: #conditon to handel error for zero division error if distance is zero.
            weights.append(0)

    return neighbors,weights
        
#referenced from :http://machinelearningmastery.com/tutorial-to-implement-k-nearest-neighbors-in-python-from-scratch/
def getResponse(neighbors,labels,weights): #returns the majority voted response from a number of neighbours
    classVotes = {}
    for x in range(len(neighbors)):
        response = labels[neighbors[x]]
        
        if response in classVotes:
            classVotes[response] += weights[x] #summing of weights is taking place for each neighbor to query x
        else:
...
def main(splitvalue,kvalue):
    ...
    print('K VALUE : ', k)
    for x in range(len(test_set)):
            neighbors,weights = getNeighbors(train_set, test_set[x], k)
            
            result = getResponse(neighbors,labels,weights)
